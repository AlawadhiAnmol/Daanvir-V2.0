...
//if($nif!=1)return;
	...
	$headline=ucfirst($rdsc['headline']); 
	$tagline=ucfirst(substr($rdsc['tagline'],0,70)); 
	...
	if($n1==1){
		...
		$stid=$r1['stateId']; 
		$locn=ucfirst(strtolower($r1['localityName']));  
	...
	if($n2==1){
		...
		$st=$stn=ucfirst(strtolower($r2['stateName']));
			}
		}
...
//	$l+=$count;
	...
	if($count>0){
        ...
        $counts=mysqli_num_rows($check22);
...
<hr class="style13"> 
				<figcaption id="FIGCAH_PTION_9">
							<h3 id="H3_12">
								<font style="font-weight:bold;font-size:20px;font-family:Helvetica;">'.$headline.'</font><br>
					...
					<div id="H_DIV_17" class="details '.$cls1.'">
						<ul id="H_UL_18">
							<li id="H_LI_19">
							'.$tagline.'
                            ....<a onclick="'.$clk.'" style="color:#2196F3">more</a>
							</li>
						</ul>
				</figcaption>
				<figure id="FIGURE_6"> 
...
//if($nif!=1)return;
    ...
    if($n1==1){
        $r1=mysqli_fetch_assoc($headq); 
        $stid=$r1['stateId']; 
        $locn=strtoupper($r1['localityName']);  
    $headq1=mysqli_query($GLOBALS['conn'],"SELECT stateName FROM states WHERE stateId='$stid'"); 
    $n2=mysqli_num_rows($headq1); 
    if($n2==1){
        $r2=mysqli_fetch_assoc($headq1);  
        $st=$stn=strtoupper($r2['stateName']);
            }
        }
...
$userDet_="";
    echo     '<li id="H_LI_4" style="height:220px;>
